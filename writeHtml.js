...
function getTagWithCDN(fileName, type = FILE_TYPE.JS) {
  const url = `https://cdn.jsdelivr.net/gh/dianluyuanli-wp/cloudDiskDist@${newVersion}/cloudDisk/${fileName}`;
...
const newContent = oldContent.replace('<app_bundle />', getTagWithCDN('app.bundle.js'))
  .replace('<common_bundle />', getTagWithCDN('common.bundle.js'))
  .replace('<common_css />', getTagWithCDN('common.css', FILE_TYPE.CSS));
//  生成最终的结果:替换cdn源之后的html文件
fs.writeFileSync('dist/cloudDisk/index.html', newContent, 'utf8');
